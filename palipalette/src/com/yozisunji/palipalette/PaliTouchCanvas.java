         ...
         public void pasteObject() 
         {
                 ...
                 for(int i=0; i<copyObject.size(); i++) {
                         ...
                         case PaliCanvas.TOOL_PENCIL:
                                 ...
                                 break;
                         case PaliCanvas.TOOL_BRUSH:                        	 	 
                                 SVGParser.Layers.get(PaliCanvas.currentLayer).objs.add(new PaliBrush(obj.svgtag, obj.bitmap, obj.rect, obj.theta, obj.f_paint, obj.filter));
                                 break;
                         case PaliCanvas.TOOL_CIRCLE:
                 ...
                 if(selector.selObjArr.size()>0) {
                         for(int i=0; i<selector.selObjArr.size(); i++) {
                                 ...
                                 case MainActivity.STYLE_FILLCOLOR:
                                         ...
                                         SVGParser.Layers.get(selector.selObjArr.get(i).x).objs.get(selector.selObjArr.get(i).y).filter = new LightingColorFilter(PaliCanvas.fillColor, 1);
                                         break;
