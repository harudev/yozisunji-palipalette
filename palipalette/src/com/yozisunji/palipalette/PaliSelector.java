...
public class PaliSelector extends View {
	...
	public PaliSelector(Context context, RectF r, PaliCanvas c, PaliTouchCanvas tc) {
		super(context);
	...
	public void onDraw(Canvas cnvs) {
		...
		cnvs.scale((1+(scaleX/cnvs.getWidth())) * PaliCanvas.zoom, (1+(scaleY/cnvs.getHeight())) * PaliCanvas.zoom);
		cnvs.save();
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (moveButton.getBounds().contains((int) downX, (int) downY)) {
				mMovePressed = true;
			} else if (rotateButton.getBounds().contains((int) downX, (int) downY)) {
				mRotatePressed = true;
			} else if (scaleButton.getBounds().contains((int) downX, (int) downY)) {
				mScalePressed = true;
			}
